from backend.interop.models import SourceDb, Entity, Registry, Mapping
...
class RegistryService:
    ...
    def get_registry_item(
        ...
        # Resolve mapping
        mapping_entry = self._resolve_mapping(
            local_id_or_uid, 
            entity_type_id, 
            resource
        )
        ...
        registry_entry = self._get_registry_entry(
            mapping_entry.source_db_id,
            mapping_entry.entity_type_id,
            mapping_entry.local_id
        )

        # Get all rows from Mapping with mapping_entry.uid
        all_mappings = self.db.session.query(Mapping).filter(
            Mapping.uid == mapping_entry.uid
        ).all()

        attribute_list = []
        for entity in all_mappings:
            source_db = self.db.session.query(SourceDb).filter(
                SourceDb.id == entity.source_db_id
            ).first()
    ...
    def _get_entity_type(self, resource: str) -> Entity:
        ...
        entity_type = self.db.session.query(Entity).filter(
    ...
    def _resolve_mapping(
        ...
        if identifier.startswith(('G-', 'S-')):
            # Search by UID
            mapping_entry = self.db.session.query(Mapping).filter(
                Mapping.uid == identifier,
                Mapping.entity_type_id == entity_type_id
            ).first()
            
            if not mapping_entry:
                raise ResourceNotFoundError(
                    f"UID '{identifier}' not found for {resource}"
                )
        else:
            # Search by local ID
            mapping_entry = self.db.session.query(Mapping).filter(
                Mapping.local_id == identifier,
                Mapping.entity_type_id == entity_type_id
            ).first()
            
            if not mapping_entry:
                raise ResourceNotFoundError(
                    f"Local ID '{identifier}' not found for {resource}"
                )
    ...
    def _get_registry_entry(
        ...
        registry_entry = self.db.session.query(Registry).filter(
