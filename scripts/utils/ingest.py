...
def generate_uid(entity_type: str, local_id: str) -> str:
    ...
    hash_input = f"{entity_type}:{local_id}".encode('utf-8')
    ...
    try:
        ...
        for i, entity in enumerate(entities, 1):
            try:
                ...
                existing_registry = session.query(Registry).filter(
                ...
                if existing_registry:
                    print(f"Registry entry already exists...")
                else:
                    ...
                    print(f"Added to registry table")
                
                # Check if mapping already exists
                existing_mapping = session.query(Mapping).filter(
                ...
                if existing_mapping:
                    ...
                    existing_mapping.updated_at = datetime.datetime.now(datetime.timezone.utc)
                    ...
                    print(f"Added to mapping table")
